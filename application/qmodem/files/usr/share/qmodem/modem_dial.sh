...
update_config()
{
    ...
    config_get use_mbim_proxy $modem_config use_mbim_proxy
...
mbim_dial(){
    ...
    # respawn on its own; we just probe pidof here and decide whether
    # to pass `-p mbim-proxy` to quectel-CM-M.
    #
    # Falls back to a plain direct dial if any precondition is missing,
    # so behaviour matches upstream QModem on builds without lpac.
    mbim_proxy_flag=""
    if [ "$use_mbim_proxy" = "1" ] \
        && [ -e "/usr/bin/quectel-CM-M" ] \
        && pidof mbim-proxy >/dev/null 2>&1; then
        mbim_proxy_flag="-p mbim-proxy"
        m_debug "mbim-proxy detected, sharing /dev/cdc-wdm* with lpac"
    fi
...
qmi_dial()
{
    ...
    # Add mbim-proxy flag if set (for eSIM/lpac coexistence)
